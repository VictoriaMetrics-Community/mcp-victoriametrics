...
func TestSelectAPIURL(t *testing.T) {
	// Create a config with a test URL
	cfg := &Config{}
	testURL, _ := url.Parse("http://example.com")
	cfg.entryPointURL = testURL

	// Test case 1: Single instance
	t.Run("Single instance", func(t *testing.T) {
		// Set instance type to single
		cfg.instanceType = "single"

		// Test SelectAPIURL
		url := cfg.SelectAPIURL("tenant1", "api", "v1", "query")

		// Check URL
		expected := "http://example.com/api/v1/query"
		if url != expected {
			t.Errorf("Expected URL '%s', got: '%s'", expected, url)
		}
	})

	// Test case 2: Cluster instance
	t.Run("Cluster instance", func(t *testing.T) {
		// Set instance type to cluster
		cfg.instanceType = "cluster"

		// Test SelectAPIURL
		url := cfg.SelectAPIURL("tenant1", "api", "v1", "query")

		// Check URL
		expected := "http://example.com/select/tenant1/prometheus/api/v1/query"
		if url != expected {
			t.Errorf("Expected URL '%s', got: '%s'", expected, url)
		}
	})

	// Test case 3: Default tenant
	t.Run("Default tenant", func(t *testing.T) {
		// Set instance type to cluster
		cfg.instanceType = "cluster"

		// Test SelectAPIURL with empty tenant
		url := cfg.SelectAPIURL("", "api", "v1", "query")

		// Check URL
		expected := "http://example.com/select/0/prometheus/api/v1/query"
		if url != expected {
			t.Errorf("Expected URL '%s', got: '%s'", expected, url)
		}
	})
...
func TestAdminAPIURL(t *testing.T) {
	...
	// Test AdminAPIURL
	url := cfg.AdminAPIURL("api", "v1", "admin")

	// Check URL
	expected := "http://example.com/api/v1/admin"
	if url != expected {
		t.Errorf("Expected URL '%s', got: '%s'", expected, url)
	}
}
